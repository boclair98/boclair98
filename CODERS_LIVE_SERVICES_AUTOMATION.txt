...
UI·UX는 모든 서비스 고도화에서 가장 중요한 제품 품질 기준이다.
기능을 추가할 때도 사용자가 화면에서 이해하고 실행하고 완료하는 경험을 함께 개선한다.
각 하루 5회 배치마다 장애·보안·데이터 손실 위험을 제외하면 UI·UX를 최우선으로 검토한다.
18:00 배치만 UI를 보는 것이 아니다. 00:00·08:00·12:00·18:00·21:00 모든 배치에서 화면 품질을 확인한다.
실제 운영 URL을 브라우저로 열어 첫 화면과 핵심 사용자 흐름을 직접 확인한다.
스크린샷이나 HTML 응답만 보고 UI가 좋다고 판단하지 않는다.
모바일 360×800·390×844, 태블릿 768×1024, 데스크톱 1440×900에서 실제 렌더링을 확인한다.
가로 스크롤, 잘린 콘텐츠, 겹치는 요소, 한 글자씩 끊기는 한국어, 작은 터치 영역을 우선 수정한다.
첫 화면에서 서비스 목적과 다음 행동이 즉시 이해되어야 한다.
정보 계층, 제목·본문 글꼴, 줄 간격, 여백, 색상, 대비, 카드·버튼·입력 상태를 일관되게 설계한다.
로딩·빈 화면·오류·성공·권한 없음·재시도·오프라인 상태를 완성도 있게 만든다.
핵심 버튼은 눈에 띄고 서로의 우선순위가 명확해야 한다.
모바일 터치 영역은 일반적으로 44 CSS px 이상으로 확보한다.
긴 한국어는 자연스럽게 줄바꿈하고 flex·grid 자식에 min-width: 0 등 필요한 수축 규칙을 적용한다.
키보드 포커스, 시맨틱 요소, 폼 라벨, 색상 대비, reduced motion을 확인한다.
디자인 토큰과 재사용 컴포넌트를 만들어 화면마다 다른 임시 스타일이 쌓이지 않게 한다.
프로젝트에 가장 적합하다면 React·Next.js·Vue·Svelte·Tailwind·CSS Modules·shadcn/ui·차트·애니메이션 등 어떤 UI 기술도 선택할 수 있다.
기술 선택은 현재 스택과 배포 환경을 확인한 뒤 사용자 경험을 실제로 좋아지게 할 때만 적용한다.
화려한 효과보다 읽기 쉬운 구조, 빠른 반응, 명확한 피드백, 신뢰감을 우선한다.
기존 기능을 깨뜨리는 대규모 프레임워크 교체는 근거·롤백 계획·검증이 있을 때만 한다.
UI 개선마다 개선 전 문제, 개선 후 동작, 사용자 가치, 반응형 검증 결과를 작업 원장에 기록한다.
UI가 개선되었다고 보고하기 전에 실제 화면과 핵심 상호작용을 확인한다.
UI 변경은 관련 기능 테스트·빌드·브라우저 오류 확인 후에만 merge·배포한다.
판매·시연·기업 검토를 가정해 빈 화면, 어색한 문구, 임시 버튼, 가짜 데이터, 미완성 상태를 남기지 않는다.
매 배치 보고에는 이번에 확인한 UI 문제, 실제 수정 여부, 남은 UI 작업을 서비스별로 구분해 기록한다.
...
GitHub에 새 저장소가 올라오면 다음 발견 배치에서 자동으로 전체 저장소 목록에 포함해 다시 판정한다.
저장소가 새로 생겼다는 사실만으로 임의의 코드를 배포하지 않는다. 해당 저장소의 관리 이력, 실제 Coders.kr production URL, Coders source 연결, 운영 화면 응답을 모두 확인한 뒤 active로 편입한다.
편입 조건을 통과한 새 서비스는 기존 서비스와 같은 원본 push → 실제 fork 생성·확인 → fork 동기화 → Coders.kr 배포 → production 검증 흐름에 자동으로 합류한다.
GitHub에는 있지만 아직 배포되지 않았거나 운영 근거가 부족한 저장소는 candidate 또는 excluded로 기록하고, 필요한 연결·API·권한을 보고한다.

운영 중이라는 사실은 고도화 작업의 종료 신호가 아니다. HTTP 200이나 현재 장애가 없다는 결과만으로 검토를 끝내지 않는다.
매 AI 배치마다 active 서비스 전체를 제품 기획 대상으로 삼아 현재 사용자 여정, 반복 불편, 차별화 가능성, 재방문·전환을 높일 기능, 기업 도입을 막는 요소를 다시 생각한다.
각 서비스마다 관찰된 사실, 검증이 필요한 제품 가설, 다음 기능·UI·운영 개선안, 완료 조건을 갱신한다.
그중 사용자 영향과 안전성이 가장 큰 작업을 실제 코드로 구현하고, 나머지도 다음 순환 대상과 우선순위를 남긴다.
특정 서비스가 계속 검토만 되고 구현에서 빠지지 않도록 순환 기록을 관리한다.
장애가 없어도 가치가 명확한 기능 고도화는 추진하되, 근거 없는 장식 기능이나 위험한 대규모 변경은 만들지 않는다.
...
- 이전 실행에 없던 새 GitHub 저장소도 같은 조건으로 매번 재평가하며, 새 active 수를 고정 목록에 추가하지 않고 발견 결과에서 계산한다.
...
그 다음 active 서비스 전체에 대해 이번 배치의 기획 상태와 다음 개선 후보를 갱신한다.
변경·장애·사용자 영향이 큰 서비스를 먼저 구현하고, 나머지 서비스는 순환 방식으로 실제 개선 기회를 계속 검토한다.
순환 기록을 통해 특정 서비스가 계속 검토에서 빠지거나 기획만 반복되지 않게 한다.
매 배치마다 모든 코드를 처음부터 다시 읽지 않는다.
이미 해결된 finding을 다시 만들지 않는다.
실제 장애가 없더라도 제품 가설과 사용자 가치가 검증된 개선은 구현한다. 안전한 구현 근거가 부족한 항목은 억지로 배포하지 말고 가설·보류 사유·다음 검증을 기록한다.
...
- 운영 장애가 없어도 사용자가 돈을 내거나 계속 사용할 이유를 높이는 핵심 기능
- 서비스별 성장 단계, 차별화, 기업 도입·판매를 막는 제품·운영·보안·계약 요소
...
5. coders-kr 저장소가 없으면 canonical 원본에서 실제 fork를 생성하고, 있으면 실제 fork인지 parent를 확인한다.
...
- 서비스별 실제 개선·장애·다음 작업
- active 전체의 이번 기획 후보, 실제 구현 여부, 보류 사유와 다음 검토 시점
- 테스트와 빌드 결과
- canonical merge 결과
